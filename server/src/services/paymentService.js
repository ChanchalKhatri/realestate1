...
export const createMyPayment = async (payment) => {
  try {
    ...
    const query = `INSERT INTO payment 
      (user_id, property_id, total_price, amount_paid, payment_method, payment_details, status, payment_date, invoice_number) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`;
    ...
    return {
      success: true,
      message: `Payment successful via ${payment.payment_method}`,
...
export const getPaymentData = async (user_id, property_id) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT p.*, pr.price as property_price FROM payment p LEFT JOIN properties pr ON p.property_id = pr.id WHERE p.user_id = ? AND p.property_id = ?",
      [user_id, property_id]
    );
    ...
    if (rows.length > 0) {
      ...
      // The total price in payment table is the 10% deposit
      const depositAmount =
        payments[0].total_price !== null ? payments[0].total_price : 0;
