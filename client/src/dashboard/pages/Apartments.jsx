...
const Apartments = () => {
  ...
  const fetchApartments = async () => {
    try {
      ...
      if (!response.ok) {
        console.error(
          `Error fetching apartments: ${response.status} ${response.statusText}`
        );
        setApartments([]);
        return;
      }
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        console.error("Unexpected API response for apartments:", result);
        setApartments([]);
      }
    } catch (error) {
  ...
  const handleMultiUnitToggle = (e) => {
    ...
    if (e.target.checked) {
      // Initialize the floor properties array when multi-unit is enabled
  ...
  const generateFloorProperties = (floors, unitsPerFloor) => {
    ...
    for (let floor = 1; floor <= floors; floor++) {
      ...
      for (let unit = 1; unit <= unitsPerFloor; unit++) {
        floorUnits.push({
          unitNumber: `${floor}${String.fromCharCode(64 + unit)}`, // 1A, 1B, 1C, etc.
          area: "",
          bedrooms: "",
          bathrooms: "",
          price: "",
  ...
  const handleAddOrEditApartment = async () => {
    try {
      ...
      // Add apartment details to form data
      Object.keys(newApartment).forEach((key) => {
        formData.append(key, newApartment[key]);
      });

      // Add multi-unit flag and properties if it's a multi-unit building
      ...
      let url = "http://localhost:3000/api/apartments";
      let method = "POST";

      if (isEditing) {
        url = `http://localhost:3000/api/apartments/${selectedApartment.id}`;
        method = "PUT";
      }
      ...
      if (result.success) {
        // Clear form and refresh apartments list
        resetForm();
        fetchApartments();
        setShowModal(false);
      } else {
      ...
      console.error(
        `Error ${isEditing ? "updating" : "adding"} apartment:`,
        error
      );
      alert(
        `Error ${isEditing ? "updating" : "adding"} apartment: ${error.message}`
      );
    }
  };

  const handleEditApartment = async (apartment) => {
    // Fetch the complete apartment data with units
    try {
      ...
      if (!result.success || !result.data) {
        throw new Error("Failed to get apartment details");
      }

      // Get the complete apartment data with units
      ...
      // Check if it's a multi-unit building
      ...
      if (isMultiUnitBuilding && hasUnits) {
        ...
        apartmentData.units.forEach((unit) => {
          ...
          if (!unitsByFloor[floorNum]) {
            unitsByFloor[floorNum] = [];
          }
          unitsByFloor[floorNum].push(unit);
        });

        // Convert to the floor properties format
        ...
        // Find the maximum number of units per floor
      ...
      alert(`Error loading apartment details: ${error.message}`);
    }
  };
